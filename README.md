...
full-stack web application that screens for **9 different health conditions** from a single interface, each powered by its own trained neural network. Built with a Flask and Streamlit backend serving TensorFlow/ models, and a vanilla HTML/CSS/JS frontend with no external frontend framework.

 **Disclaimer:** This is educational / portfolio project. Not all training data is real-world clinical data — some datasets are synthetic or semi-synthetic and used strictly for learning purposes. **This tool does not replace clinical judgment, laboratory diagnosis, or professional medical advice**, and should not be used for real diagnostic decisions.

---

## What it does

Pick  condition from the tab bar, fill in the patient's values (or click "Fill sample values" to autofil realistic example), and run the model. Each tab's form fields map 1:1 to the exact feature colums of that model's training dataset — nothing is guesed or reformatted on the way in.

The backend runs the real trained model for every request and returns  risk score,  plain-language message,  recommendation, and — for the two multi-class models —  specific predicted diagnosis label.
...
| 3 | Stroke | Binary (NN) | 93.45% | **0.00%**  | 0.6410 |
...
| 8 | Chronic Kidney Disease (CKD) | Binary (NN) | **100.00%**  | **100.00%**  | 0.0882 |
